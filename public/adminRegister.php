...
if (isset($_POST['submit_button'])) {
    ...
    if($c_ty!=-8 && $c_untry!=-8 && $n_mber!=-8 && $p_stalcode!=-8 && $t_rms!=-8 && $g_nder!=-8 && $pa_sword!=-8 && $e_ail!=-8 && $n_me!=-8 && $con_password!=-8){
        $valueiinsert=$Connection->prepare('INSERT INTO Admin(email,name,phone_number,password,city,country,postal_code,gender)VALUES(:emai,:names,:numbr,:passw,:cit,:count,:post,:gend)');
        $user_criteri_a=[
            ':emai'=>$e_ail,
            ':names'=>$n_me,
            ':numbr'=>$n_mber,
